...
export const requestLimiter = (token: string, res: any, next: any) => {
  // check that redis client exists
  if (!redisClient) {
    throw new Error('Redis client does not exist!');
  }

  // fetch records of current user using jwt token, returns null when no record is found
  redisClient.get(token, function (err, record) {
    if (err) throw err;
    ...
    if (record === null) {
      ...
      redisClient.set(token, JSON.stringify(newRecord));
      next();
    } else {
